...
#include <map>			//for map
#include <iterator>		//for map iterator
...
int main(int argc, char* argv[]){
	...
	*/
	...
	//counting number of page faults for each process

	ifstream fin;
	ofstream fout2;

	fin.open("file2.txt");
	fout2.open("file1.txt");

	int pos, procId;
	string strForLine, sub;

	map <int, int> mapForProcCount;
	map <int, int>::iterator itr;

	getline(fin, strForLine);
	getline(fin, strForLine);
	
	while(fin.eof() == 0){
		getline(fin, strForLine);
		
		if(strForLine.size() > 0){
			pos = strForLine.find(",");
			sub = strForLine.substr(1, pos-1);
			procId = atoi(sub.c_str());
			mapForProcCount[procId]++;
		}
	}

	for(itr = mapForProcCount.begin(); itr!=mapForProcCount.end(); itr++){
		fout2<<"Total number of page faults for process with process id : "<<itr->first<<" is "<<itr->second<<"\n";
	}
	fout2<<'\n';
